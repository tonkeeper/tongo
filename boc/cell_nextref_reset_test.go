...
// TestNextRefShallowReset pins the fix for the super-linear (~O(n*depth)) block
// decode that hung the indexer: NextRef must reset only the immediate child's
// read cursors, NOT recurse into the whole subtree. A deeper cell is reset when
// it is itself descended into, so the previous full-subtree reset on every
// descent re-walked already-reset cells.
