...
type Bits288 [36]byte

func (u Bits288) FixedSize() int {
	return 288
}

func (u Bits288) MarshalJSON() ([]byte, error) {
	return []byte(fmt.Sprintf("\"%x\"", u[:])), nil
}

func (u *Bits288) UnmarshalJSON(b []byte) error {
	bs, err := hex.DecodeString(strings.Trim(string(b), "\""))
	if err != nil {
		return err
	}
	if len(bs) != 36 {
		return fmt.Errorf("can't parse Bits288 %v", string(b))
	}
	copy(u[:], bs)
	return nil
}
